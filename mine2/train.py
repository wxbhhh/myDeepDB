...
def predict(model, data_loader, cuda):
    ...
    for batch_idx, data_batch in enumerate(data_loader):
        ...
        samples, predicates, joins, counts, targets, sample_masks, predicate_masks, join_masks, count_masks = data_batch

        if cuda:
            samples, predicates, joins, counts, targets \
                = samples.cuda(), predicates.cuda(), joins.cuda(), counts.cuda(), targets.cuda()
            sample_masks, predicate_masks, join_masks, count_masks \
                = sample_masks.cuda(), predicate_masks.cuda(), join_masks.cuda(), count_masks.cuda()
        samples, predicates, joins, counts, targets \
            = Variable(samples), Variable(predicates), Variable(joins), Variable(counts), Variable(targets)
        sample_masks, predicate_masks, join_masks, count_masks \
            = Variable(sample_masks), Variable(predicate_masks), Variable(join_masks), Variable(count_masks)

        t = time.time()
        outputs = model(samples, predicates, joins, counts, sample_masks, predicate_masks, join_masks, count_masks)
        t_total += time.time() - t
...
def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda):
    ...
    model = SetConv(sample_feats, predicate_feats, join_feats, count_feats, hid_units)
    ...
    # Load test data
    file_name = "../data/test/" + workload_name
    ...
    counts_test = encode_counts(samples)
    ...
    test_data = make_dataset(samples_test, predicates_test, joins_test, counts_test, labels_test, max_num_joins, max_num_predicates)
    test_data_loader = DataLoader(test_data, batch_size=batch_size)
    ...
    # Write predictions
    file_name = "results/predictions_" + workload_name + "_" + str(int(time.time())) + "_m2.csv"
