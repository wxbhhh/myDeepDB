...
class my_nn(nn.Module):
    def __init__(self, sample_feats, predicate_feats, join_feats, label_units_num, hid_units):
        super(my_nn, self).__init__()
        self.sample_mlp1 = nn.Linear(sample_feats, hid_units)
        self.sample_mlp2 = nn.Linear(hid_units, hid_units)
        self.sample_mlp3 = nn.Linear(hid_units, hid_units)
        ...
        self.out_mlp2 = nn.Linear(hid_units, label_units_num)
    ...
    def forward(self, samples, predicates, joins, sample_mask, predicate_mask, join_mask):
        ...
        # hid_sample = F.relu(self.sample_mlp3(hid_sample))
        hid_sample = hid_sample * sample_mask  # Mask
        if len(samples.shape) > 2:
            hid_sample = torch.sum(hid_sample, dim=1, keepdim=False)
            sample_norm = sample_mask.sum(1, keepdim=False)
        else:
            hid_sample = torch.sum(hid_sample, dim=0, keepdim=False)
            sample_norm = sample_mask.sum(0, keepdim=False)
        ...
        if len(predicates.shape) > 2:
            hid_predicate = torch.sum(hid_predicate, dim=1, keepdim=False)
            predicate_norm = predicate_mask.sum(1, keepdim=False)
        else:
            hid_predicate = torch.sum(hid_predicate, dim=0, keepdim=False)
            predicate_norm = predicate_mask.sum(0, keepdim=False)
        ...
        if len(joins.shape) > 2:
            hid_join = torch.sum(hid_join, dim=1, keepdim=False)
            join_norm = join_mask.sum(1, keepdim=False)
        else:
            hid_join = torch.sum(hid_join, dim=0, keepdim=False)
            join_norm = join_mask.sum(0, keepdim=False)
        hid_join = hid_join / join_norm

        if len(samples.shape) > 2:
            hid = torch.cat((hid_sample, hid_predicate, hid_join), 1)
        else:
            hid = torch.cat((hid_sample, hid_predicate, hid_join), 0)
