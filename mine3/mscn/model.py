...
class SetConv(nn.Module):
    def __init__(self, sample_feats, predicate_feats, join_feats, count_feats, hid_units):
        ...
        self.sample_mlp1 = nn.Linear(sample_feats + 1, hid_units)
    ...
    def forward(self, samples, predicates, joins, counts, sample_mask, predicate_mask, join_mask, count_mask):
        ...
        hid_sample = torch.transpose(counts, 2, 1)
        hid_sample = torch.cat((samples, hid_sample), dim=2)
  ...
  #      hid_sample = F.relu(self.sample_mlp1(samples))
        ...
        hid_count = torch.sum(counts, dim=1, keepdim=False)
