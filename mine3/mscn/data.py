...
def load_and_encode_train_data(num_queries, num_materialized_samples):
    file_name_queries = "../data/train/train"
    file_name_column_min_max_vals = "../data/train/column_min_max_vals.csv"
    ...
    counts_enc = encode_counts(samples)
    ...
    counts_train = counts_enc[:num_train]
    ...
    counts_test = counts_enc[num_train:num_train + num_test]
    ...
    train_data = [samples_train, predicates_train, joins_train, counts_train]
    test_data = [samples_test, predicates_test, joins_test, counts_test]
...
def make_dataset(samples, predicates, joins, counts, labels, max_num_joins, max_num_predicates):
    """Add zero-padding and wrap as tensor dataset."""

    count_masks = []
    count_tensors = []
    for count in counts:
        count_tensor = np.vstack(count)
        num_pad = max_num_joins + 1 - count_tensor.shape[0]
        count_mask = np.ones_like(count_tensor).mean(1, keepdims=True)
        count_tensor = np.pad(count_tensor, ((0, num_pad), (0, 0)), 'constant')
        count_tensor = np.transpose(count_tensor)
        count_mask = np.pad(count_mask, ((0, num_pad), (0, 0)), 'constant')
        count_mask = np.transpose(count_mask)
        count_tensors.append(np.expand_dims(count_tensor, 0))
        count_masks.append(np.expand_dims(count_mask, 0))
    count_tensors = np.vstack(count_tensors)
    count_tensors = torch.FloatTensor(count_tensors)
    count_masks = np.vstack(count_masks)
    count_masks = torch.FloatTensor(count_masks)
    ...
    return dataset.TensorDataset(sample_tensors, predicate_tensors, join_tensors, count_tensors, target_tensor, sample_masks,
                                 predicate_masks, join_masks, count_masks)
